...
use DoctrineExtensions\Common\Mapping\Driver\AbstractFile;
 ...
 */
class Yaml extends AbstractFile implements DriverInterface
{
    /**
     * File extension
     * @var string
                ...
                if (isset($fieldMapping['gedmo'])) {
                    if (in_array('treeLeft', $fieldMapping['gedmo'])) {
                        ...
                        if (strlen($separator) > 1) {
                            throw new InvalidMappingException("Tree TreePath field - [{$field}] Separator {$separator} is invalid. It must be only one character long.");
                        }
                        ...
                        if (!$validator->isValidFieldForLocktime($meta, $field)) {
                            throw new InvalidMappingException("Tree TreeLockTime field - [{$field}] type is not valid. It must be \"date\" in class - {$meta->name}");
                        }
