...
namespace DoctrineExtensions\Tree\Document\Repository;
...
abstract class AbstractTreeRepository extends DocumentRepository implements RepositoryInterface
{
    ...
    public function __construct(DocumentManager $em, UnitOfWork $uow, ClassMetadata $class)
    {
        ...
        foreach ($em->getEventManager()->getListeners() as $listeners) {
            foreach ($listeners as $listener) {
                if ($listener instanceof \DoctrineExtensions\Tree\TreeSubscriber) {
        ...
        if (is_null($treeListener)) {
            throw new \DoctrineExtensions\Common\Exception\InvalidMappingException('This repository can be attached only to ODM MongoDB tree listener');
        }

        $this->listener = $treeListener;
        if (!$this->validate()) {
            throw new \DoctrineExtensions\Common\Exception\InvalidMappingException('This repository cannot be used for tree type: '.$treeListener->getStrategy($em, $class->name)->getName());
        }
     ...
     *
     * @param \DoctrineExtensions\Tree\RepositoryUtilsInterface $repoUtils
     *
     * @return $this
     */
     ...
     *
     * @return \DoctrineExtensions\Tree\RepositoryUtilsInterface|null
     */
    ...
    /**
     * @see \DoctrineExtensions\Tree\RepositoryUtilsInterface::setChildrenIndex
     */
    ...
    /**
     * @see \DoctrineExtensions\Tree\RepositoryUtilsInterface::getChildrenIndex
     */
